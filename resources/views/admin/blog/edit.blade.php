...
<body>
  <div class="container pt-5">
    <div class="row pt-5 pb-5">
      ...
      <form class="login-form p-5" method="POST" action="{{ route('admin.blogs.update', ['blog' => $blog]) }}"
        ...
        <div class="row">
          <div class="col-sm-12 col-md-6">
            ...
            <input type="date" name="ngayBlog" value="{{ $blog->ngayBlog }}"><br>

            <label for="Header">Header:</label><br>
            <input type="text" name="header"value="{{ $blog->header }}"><br>

            <label for="shortInfo">Mô tả ngắn gọn:</label><br>
            <textarea name="shortInfo">{{ $blog->shortInfo }}</textarea><br>

            @for ($i = 1; $i <= 5; $i++)
              <label for="deltailInfo">Chi tiết đoạn {{ $i }}:</label><br>
              <textarea name="deltailInfo{{ $i }}">{{ $blog->{'deltailInfo' . $i} }}</textarea>
